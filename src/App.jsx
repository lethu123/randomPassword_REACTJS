...
export class App extends Component {
  ...
  rdPassword = ({ number, upper, lower, len, symbol }) => {
    ...
    let finalPassword = '';
    for (let i = 0; i <= len; i++) {
      const a = checkedOptions[Math.floor(Math.random() * checkedOptions.length)];
        ...
        <div className="content">
          <form onSubmit={this.handleSubmit}>
            <div className="d-flex">
              <input type="text" name="result" className="inputText" onChange={this.handleChange} ref={this.input} value={result} />
              <div className="tooltip">
                <button type="button" className="btnCoppy" onClick={this.myFunction} onMouseOut={this.outFunc} >
                  <span className="tooltiptext" id="myTooltip">Copy to clipboard</span>
                  coppy</button>
              </div>
            </div>
            <div style={{ marginTop: "20px" }}>
              <input type="checkbox" onChange={this.handleCheck} value={number} name="number" id="number" />
              <label htmlFor="number">number</label>
            </div>
            <div>
              <input type="checkbox" onChange={this.handleCheck} name="upper" value={upper} id="upper" />
              <label htmlFor="upper">uppercase</label>
            </div>
            <div>
              <input type="checkbox" onChange={this.handleCheck} name="symbol" value={symbol} id="symbol" />
              <label htmlFor="symbol">symbol</label>
            </div>
            <div>
              <input type="checkbox" onChange={this.handleCheck} name="lower" value={lower} id="lower" />
